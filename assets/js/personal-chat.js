...
async function init(current) {
  ...
  const query = firestore.query(firestore.collection(db, 'directConversations'), firestore.where('participants', 'array-contains', user.uid), firestore.orderBy('updatedAt', 'desc'));
  ...
  if (requestedUid && requestedUid !== user.uid) {
    const requestedProfile = await firestore.getDoc(firestore.doc(db, 'users', requestedUid));
    if (requestedProfile.exists()) {
      const profile = requestedProfile.data();
      await showConversation({ uid: requestedUid, name: profile.name || 'Member', avatar: profile.profilePicture || profile.photoURL || '' }, firestore);
    }
  }
  nicknameButton.onclick = async () => { if (!target || !activeConversationId) return; const value = prompt('Set a nickname for this person:', target.nickname || target.name); if (value === null) return; const nickname = value.trim().slice(0, 40); try { await firestore.updateDoc(firestore.doc(db, 'directConversations', activeConversationId), { nicknames: { [target.uid]: nickname }, updatedAt: firestore.serverTimestamp() }); target.nickname = nickname; name.textContent = nickname || target.name; status.textContent = nickname ? 'Nickname updated.' : 'Nickname removed.'; } catch { status.textContent = 'Nickname could not be updated.'; } };
