...
async function init() {
  ...
  const { addDoc, arrayRemove, arrayUnion, collection, deleteDoc, getDocs, limit, onSnapshot, orderBy, query, serverTimestamp, updateDoc, doc } = await import('https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js');
  const profileModal = document.getElementById('profileModal'); const profileBody = document.getElementById('profileModalBody'); const messageModal = document.getElementById('messageModal'); const messageAvatar = document.getElementById('messageAvatar'); const messageList = document.getElementById('messageList'); const messageForm = document.getElementById('messageForm'); const messageText = document.getElementById('messageText'); const messageStatus = document.getElementById('messageStatus'); let messageTarget = null; let stopMessages = null;
  function openMessages(target) { if (!currentUser) { status.textContent = 'Sign in to send a private message.'; return; } if (target.uid === currentUser.uid) { status.textContent = 'You cannot message yourself.'; return; } messageTarget = target; const conversationId = [currentUser.uid, target.uid].sort().join('_'); messageModal.classList.remove('hidden'); document.getElementById('messageModalTitle').textContent = target.name; messageAvatar.innerHTML = target.avatar ? `<img src="${escapeHtml(target.avatar)}" alt="">` : initials(target.name); if (stopMessages) stopMessages(); stopMessages = onSnapshot(query(collection(db, 'directMessages', conversationId, 'messages'), orderBy('createdAt', 'asc'), limit(100)), snapshot => { messageList.innerHTML = snapshot.docs.length ? snapshot.docs.map(item => { const message = item.data(); return `<div class="private-message ${message.senderId === currentUser.uid ? 'mine' : ''}">${escapeHtml(message.text)}</div>`; }).join('') : '<p class="comments-empty">No messages yet.</p>'; messageList.scrollTop = messageList.scrollHeight; }, () => { messageStatus.textContent = 'Messages could not be loaded.'; }); }
  ...
  messageForm.addEventListener('submit', async event => { event.preventDefault(); const value = messageText.value.trim(); if (!value || !messageTarget || !currentUser) return; const conversationId = [currentUser.uid, messageTarget.uid].sort().join('_'); messageStatus.textContent = 'Sending...'; await addDoc(collection(db, 'directMessages', conversationId, 'messages'), { senderId: currentUser.uid, receiverId: messageTarget.uid, text: value.slice(0, 1000), createdAt: serverTimestamp() }).then(() => { messageText.value = ''; messageStatus.textContent = ''; }).catch(() => { messageStatus.textContent = 'Message failed. Try again.'; }); });
  document.querySelectorAll('[data-close-modal]').forEach(button => button.addEventListener('click', () => { profileModal.classList.add('hidden'); messageModal.classList.add('hidden'); if (stopMessages) { stopMessages(); stopMessages = null; } }));
  ...
  feed.addEventListener('click', async event => {
    const button = event.target.closest('[data-post-action], [data-comment-action]');
    const card = button?.closest('[data-post-id]');
    const post = postItems.find(item => item.id === card?.dataset.postId);
    if (!button || !post) return;
    if (button.dataset.commentAction) { const commentId = button.closest('[data-comment-id]')?.dataset.commentId; const commentRef = doc(db, 'communityPosts', post.id, 'comments', commentId); if (button.dataset.commentAction === 'delete' && confirm('Delete this comment?')) await deleteDoc(commentRef).then(() => { button.closest('[data-comment-id]').remove(); status.textContent = 'Comment deleted.'; }).catch(() => { status.textContent = 'Comment delete failed.'; }); if (button.dataset.commentAction === 'edit') { const current = button.closest('[data-comment-id]').querySelector('span').textContent; const value = prompt('Edit comment:', current); if (value?.trim()) await updateDoc(commentRef, { text: value.trim().slice(0, 500) }).then(() => { button.closest('[data-comment-id]').querySelector('span').textContent = value.trim().slice(0, 500); status.textContent = 'Comment updated.'; }).catch(() => { status.textContent = 'Comment update failed.'; }); } return; }
    ...
    if (button.dataset.postAction === 'comment') {
      ...
      if (!panel.classList.contains('hidden')) { const comments = await getDocs(query(collection(db, 'communityPosts', post.id, 'comments'), orderBy('createdAt', 'asc'), limit(50))); panel.querySelector('.comments-list').innerHTML = comments.docs.length ? comments.docs.map(item => { const comment = { id: item.id, ...item.data() }; const canManage = currentUser?.uid === comment.authorUid; return `<div class="community-comment" data-comment-id="${comment.id}"><strong>${escapeHtml(comment.authorName || 'Visitor')}</strong><span>${escapeHtml(comment.text)}</span>${canManage ? `<button type="button" data-comment-action="edit">Edit</button><button type="button" data-comment-action="delete">Delete</button>` : ''}</div>`; }).join('') : '<p class="comments-empty">No comments yet.</p>'; } }
