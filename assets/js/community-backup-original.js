...
if (feed) {
  feed.innerHTML = '<div class="feed-empty"><p>✓ Module loaded successfully. Feed element found.</p></div>';
  console.log('[Community] Feed element found, updating HTML');
} else {
...
async function init() {
  ...
  console.log('[Community] Posts query created, starting load sequence...');
  
  // Timeout wrapper for getDocs to prevent hanging
  ...
  const showLoading = () => {
    if (!feedLoaded && feed.innerHTML.includes('Loading')) return; // Don't override loading
    feed.innerHTML = `<div class="feed-empty"><i class="fa-solid fa-spinner fa-spin"></i><p>Loading community...</p></div>`;
    console.log('[Community] Showing loading state');
  };
  
  // Show error state
  const showError = (msg, canRetry = true) => {
    feedLoaded = true;
    feed.innerHTML = `<div class="feed-empty"><i class="fa-solid fa-triangle-exclamation"></i><p>${msg}</p>${canRetry ? '<button class="retry-button" type="button" style="margin-top: 12px; padding: 8px 16px; background: #c084fc; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">Retry</button>' : ''}</div>`;
    status.textContent = `Error: ${msg}`;
    console.log('[Community] Showing error:', msg);
    
    if (canRetry) {
      feed.querySelector('.retry-button')?.addEventListener('click', () => {
        feedLoaded = false;
        status.textContent = '';
        loadPosts();
      });
  ...
  const showEmpty = () => {
    ...
    console.log('[Community] Showing empty state');
  };
  
  // Try immediate load first
  const loadPosts = async () => {
    console.log('[Community] loadPosts() called');
    showLoading();
    ...
    try {
      ...
      loadTimeout = setTimeout(() => {
        console.log('[Community] Fallback timeout fired, feedLoaded =', feedLoaded);
        if (!feedLoaded) {
          showError('Taking longer than expected. Check your internet connection.', true);
        }
      }, 7000);
      
      // Wrap getDocs with a timeout
      console.log('[Community] Calling getDocs...');
      const snapshot = await fetchWithTimeout(getDocs(postsQuery), 6000);
      clearTimeout(loadTimeout);
      ...
      if (!feedLoaded) {
        ...
        console.log('[Community] Showing error from catch block:', errorMsg);
        showError(errorMsg, true);
      } else {
        console.log('[Community] feedLoaded already true, not showing error');
      }
    }
  };
  
  // Start loading immediately
  console.log('[Community] About to call loadPosts()');
  
  // SIMPLE TEST: Check if we reach this point
  feed.innerHTML = '<div class="feed-empty"><p style="color: yellow;">DEBUG: loadPosts() about to be called...</p></div>';
  
  loadPosts();
  
  // Also set up real-time listener for future updates
  const unsubscribe = onSnapshot(postsQuery, snapshot => { 
    console.log('[Community] Real-time listener update:', snapshot.docs.length, 'docs');
    ...
    } else if (postItems.length > 0 || snapshot.docs.length > 0) {
      // Update existing posts from real-time listener
      postItems = snapshot.docs.map(item => ({ id: item.id, ...item.data() }));
      if (postItems.length > 0) {
        render();
        console.log('[Community] Posts updated from real-time listener');
      }
    }
  }, error => { 
